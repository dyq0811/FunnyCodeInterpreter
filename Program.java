...
public class Program {
    ...
    {
        Deque<Character> deque = new CircularArrayDeque<Character>();
        int lineNum = 0;
        while(mScanner.hasNextLine())
        {
            ...
            for (int i = 0; i < line.length(); i++)
            {
                ...
                {
                    if (match(deque.getLast(), ch))
                    {
                        deque.removeLast();
                    }
                    else
                    {
                        throw new SyntaxErrorException("Syntax error is encountered at line "
                            + lineNum + ": " + "mismatching " + deque.getLast() + " and " + ch);
                    }
                }
            }
            lineNum++;
        ...
        throw new SyntaxErrorException("Syntax error is encountered: extra bracket(s)" 
            + "and the last bracket is" + deque.getLast());
    }
    ...
    public void step(SimulationComponent mComp) {
        ...
        if(!gStarted && callStack != null) {
            callStack.addLast(gMethodList.get(0));
            gStarted = true;
        }
        if(callStack != null && !callStack.isEmpty()) { 
            if(callStack.getLast().isFinished()) { 
                callStack.removeLast(); 
            } else { 
                callStack.getLast().step(mComp); 
            }
        }
        else { 
            bFinished = true; 
